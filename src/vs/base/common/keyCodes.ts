...
export namespace JSONKeysUtils {
	...
	export function toString(key: JSONKey) {
		...
		if (key.type === 'mouse' && key.times === undefined) {
			// Reset "times", so comparing two strings generated by this function, one with `times: undefined` and one
			// with `times: 1` (which are identical), will give the correct result
			key.times = 1;
		}
